...
sql::Connection* GeneralOperations::mysqlConnect(string database)
{
    ...
    try
	{
        ...
        con = driver->connect(DBServer,Username,Password);	
...
sql::Connection* GeneralOperations::DBConnectionWrite(string db)
{
	sql::Connection *con=NULL;
    try{
		sql::Driver *driver;
...
sql::Connection* GeneralOperations::DBConnectionRead(string db)
{
	sql::Connection *con=NULL;
    try{
        sql::Driver *driver;
        driver = get_driver_instance();
        con = driver->connect(DBServer2,Username,Password);  
        con->setSchema(db);     
    }
    catch(exception &e)
    {
