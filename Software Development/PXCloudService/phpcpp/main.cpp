...
Updated validation.cpp with ascii values to check if the character is accepted
Inner try-catch for better error management and to continue the posting even if there is exception in a particular row.

Updates parking_movements_iot to insert/delete records in the open_transactions table
 */
...
Json::Value UpdateParkingOccupancy(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {  
    ...
    Json::Value table_rules,error_data,jsonerror;
    string sql = "";
...
Json::Value UpdateDeviceAlarms(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {   
    ...
    Json::Value jsonresponse, jsonresponseid,table_rules,error_data,jsonerror;
    int result = 0;
    int j = 0;    
    try 
    {
...
Json::Value UpdateDeviceStatus(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) 
    ...
    sql::Statement *stmt_write;
    sql::ResultSet *res;
    Json::Value data, jsonresponseid,table_rules,error_data,jsonerror;
    int carpark_number;       
    int device_number;
    string device_name;
    int device_type;
    ...
    int id=0, watchdog_device_id = 0, j = 0;
    int result = 0;
    string sql = "";
    try 
        {
...
Json::Value UpdateOpenTransactions(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {
    ...
    string parking_rate;
    int movement_type;
    Json::Value jsonresponseid, jsonresponsedelete,table_rules,error_data,jsonerror;
    int result = 0;
    int j = 0, k = 0, rowid;
...
Json::Value UpdateParkingMovementsAccess(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {    
    ...
    string device_name;
    int movement_type;
    string parking_zone,ticket_id;
    int carpark_number;    
       
    Json::Value jsonresponseid,table_rules,error_data,jsonerror;
    int result = 0;
    int j = 0;
        ...
        for (int i = 0; i < (signed)data.size(); i++) {
            try
            {
                ...
                else
                {
                    ...
                    sql = "INSERT INTO parking_movements_access (date_time, tag_serial, plate_number, tag, status_message, epc, tid, epc_count, epc_list, device_number, device_name, movement_type, parking_zone, carpark_number, carpark_name, facility_number, facility_name,carpark_id,ticket_id) VALUES('" + date_time + "','" + tag_serial + "','" + plate_number + "','" + tag + "','" + status_message + "','" + epc + "','" + tid + "'," + to_string(epc_count) + ",'" + epc_list + "'," + to_string(device_number) + ",'" + device_name + "'," + to_string(movement_type) + ",'" + parking_zone + "'," + to_string(carpark_number) + ",'" + carpark_name + "'," + facility_number+ ",'" + facility_name + "'," + to_string(carpark_id) + ",'"+ticket_id+"')";
                    result = stmt_write->executeUpdate(sql);
...
Json::Value UpdateHourlyOccupancy(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {   
    ...
    int h[24], j, k = 0;    
    int id=0, occupancy_id=0, result;
...
Json::Value UpdateParkingRevenue(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {    
    Json::Value response,error_data,jsonerror;
    sql::Connection *con_read;
    sql::Connection *con_write;
    sql::Statement *stmt;
    sql::Statement *stmt_write;
    ...
    int id=0, result, k = 0, parking_id,carpark_id;
...
Json::Value UpdateAccessWhitelist(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) 
    ...
    sql::Statement *stmt;
    sql::ResultSet *res;
    Json::Value data, jsonresponseid, jsonresponse,table_rules,error_data,jsonerror;
    string sql = "";
    
    int id=0, result = 0, j = 0, rowid = 0;
...
Json::Value UpdateParkingReservation(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {
    ...
    sql::Statement *stmt;
    sql::ResultSet *res;
    Json::Value data, jsonresponseid, jsonresponse,table_rules,jsonerror,error_data;
    string sql = "";    
    int id=0, result = 0, j = 0, rowid = 0;
...
Json::Value UpdateParkingSubscription(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {
    ...
    try 
    {
        ...
        for (int i = 0; i < (signed)data.size(); i++) 
        {
            ...
            {
                WriteException("UpdateParkingSubscription", e.what());
                error_data["id"] = id;
                error_data["message"] = e.what(); 
                jsonerror.append(error_data);
                jsonresponse["error"] =  jsonerror;      
...
Json::Value UpdateParkingMovementsIOT(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name,string ipaddress) {
    ...
    string sql = "";
    int id=0, result = 0, j = 0;    
    int device_number,movement_type,plate_capture_id,carpark_number,parking_id,status_code,iot_mode,entry_type;
               ...
               // WriteToLog("UpdateParkingSubscription", fw.write(data[i]));
                ...
                {                  
                    ...
                    if(iot_mode==0)
                        ...
                        if(movement_type==1)
                            {
                            sql = "INSERT into open_transactions(carpark_id,device_name,device_number,ticket_id,carpark_number,facility_number,entry_date_time,plate_number,entry_grace_period,entry_type,plate_captured_id,carpark_name,movement_type) VALUES("+to_string(carpark_id)+",'" + device_name + "'," + to_string(device_number) + ",'" + ticket_id + "'," + to_string(carpark_number) + "," + facility_number + ",'" + date_time + "','" + plate_number + "',DATE_ADD(entry_date_time,INTERVAL 15 MINUTE),"+to_string(entry_type)+"," + to_string(plate_capture_id) + ",'" + carpark_name + "',1)";
                            result = stmt_write->executeUpdate(sql);   
                            }
                        if(movement_type==2)
                            {
                            sql="Delete from open_transactions where facility_number="+facility_number+" and ticket_id like 'MB " + user_id + " %' and entry_type="+to_string(entry_type);
                            result = stmt_write->executeUpdate(sql);   
                            }
                        }
                }
...
Json::Value UpdateValetParking(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name) {
   // WriteToLog("here","");
    ...
    Json::Value jsonresponseid,error_data,jsonerror;
    int result = 0;
    int j = 0, rowid;
    string sql;
    ...
    try 
    {
        ...
        for (int i = 0; i < (signed)data.size(); i++) 
        {
			...
			//WriteToLog("","update valet_parking set plate_number='"+plate_number+"',plate_prefix='"+plate_prefix+"',front_plate='"+front_plate+"',left_plate='"+left_plate+"',right_plate='"+right_plate+"',back_plate='"+back_plate+"',plate_country='"+plate_country+"',plate_emirate='"+plate_emirate+"',"+sql);
            ...
            {
                WriteException("UpdateValetParking", e.what());
                error_data["id"] = id;
                error_data["message"] = e.what(); 
                jsonerror.append(error_data);
                jsonresponse["error"] =  jsonerror;   
    ...
    {
        WriteException("UpdateValetParking", e.what());
        WriteException("UpdateValetParking", sql);
...
Json::Value UpdateAccessRequestIOT(Json::Value json,string facility_number,string facility_name,Php::Value cloud_carpark_id,Php::Value cloud_carpark_name,string ipaddress) {   
    ...
    string sql = "";
    int id=0;
    string token_request_date_time,token_response_date_time,token_response,api_request_date_time,api_response_date_time,api_request,api_response,response_status,ticket_id,plate_number,iot_mode_name,carpark_name,device_name;
    int plate_captured_id,iot_mode,api_response_delay,carpark_number,device_number;
        ...
        for (int i = 0; i < (signed)data.size(); i++) 
        {
            try 
            {
                ...
                else
                {
                    ...
                    sql = "INSERT into access_request_iot(api_request_date_time,api_response_date_time,api_response_delay,api_request,api_response,response_status,ticket_id,plate_number,plate_captured_id,iot_mode,iot_mode_name,ip_address,facility_number,facility_name,carpark_number,carpark_name,carpark_id,device_number,device_name) VALUES(" +  mysqldate(api_request_date_time) + "," + mysqldate(api_response_date_time) + ","+to_string(api_response_delay)+",'" + api_request + "','" +api_response + "','" +response_status + "','" + ticket_id + "','" + plate_number + "',"+to_string(plate_captured_id)+","+to_string(iot_mode)+",'"+iot_mode_name+"','"+ipaddress+"',"+facility_number+",'"+facility_name+"',"+to_string(carpark_number)+",'"+carpark_name+"',"+to_string(carpark_id)+","+to_string(device_number)+",'"+device_name+"')";
                    result = stmt_write->executeUpdate(sql);
                    if (result == 1) 
                    {
                        jsonresponseid[j] = id;
    ...
    try {
        ...
        for (int i = 0; i < (signed)json["table"].size(); i++) {
            ...
            if (json["table"][i]["table_name"] == "access_whitelist") 
                ...
                conn = general.DBConnectionRead(DBServer);
                stmt = conn->createStatement();
		//WriteToLog("","Select * from plates_ewallet where operator_id = " + cloud_operator_id + " and  !find_in_set("+facility_id+",download_flag) limit " + to_string(rowlimit));
