//sudo g++ -std=c++11 -Wall -o PX_CloudDownload_V01_03_02_09242020 PX_CloudDownload_V01_03_02_09242020.cpp PX_GeneralOperations.cpp -ljsoncpp -lmysqlcppconn -lcurl -I/opt/lampp/include/ -L/opt/lampp/lib/ -pthread
...
void DownloadCloudData(string response)
{
	...
	string plate_number,description,carpark_number,country,user_qrcode,start_date,expiry_date,access_zones,creation_date,last_visited,company_name,customer_name,tag,tag_serial,tag_tid;
    ...
    try
    {
        ...
        if(parse)
        {
            ...
            for(i = 0;i<(signed)array["access_whitelist"].size();i++)
            {
                ...
                user_qrcode = array["access_whitelist"][i]["user_qrcode"].asString();
                ...
                {						
                    ...
                    {
                        query="Update access_whitelist set carpark_number='"+carpark_number+"',plate_number = '"+plate_number+"',country = '"+country+"',ticket_id = '"+user_qrcode+"',last_sync = CURRENT_TIMESTAMP(),wallet_user_id = "+to_string(wallet_user_id)+",validity_start_date  = "+start_date+",validity_expiry_date="+expiry_date+",wallet_payment = "+to_string(wallet_payment)+",season_card = "+to_string(season_card)+",corporate_parker="+to_string(corporate_parker)+",antipassback_enabled="+to_string(antipassback)+",access_zones='"+access_zones+"',creation_date_time = '"+creation_date+"',status="+to_string(status)+",last_visited_date_time="+last_visited+",company_name='"+company_name+"',customer_name='"+customer_name+"',tag='"+tag+"',tag_serial='"+tag_serial+"',tag_tid='"+tag_tid+"',description='"+description+"' where access_whitelist_id = "+to_string(rowid);    
                        // WriteToLog("DownloadCloudData","Query:"+query);
...
int main(void)
{
	...
	else 
    {
       ...
       pid_t pid,sid;
		...
		if(pid > 0)
		{
			WriteToLog("Main: " , "Cloud Download Daemon Version : PX_CloudDownload_V01_03_02_09242020    Process Id:"+to_string(pid));
			cout<<"Cloud Download Daemon Version : PX_CloudDownload_V01_03_02_09242020"<<"Process Id :"<<pid<<endl;	
