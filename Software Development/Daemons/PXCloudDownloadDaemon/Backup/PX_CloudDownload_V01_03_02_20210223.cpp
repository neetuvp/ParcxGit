//g++ -std=c++11 -Wall -o PX_CloudDownload_V01_03_02_20211006 PX_CloudDownload_V01_03_02_20210223.cpp PX_GeneralOperations.cpp -ljsoncpp -lmysqlcppconn -lcurl -I/opt/lampp/include/ -L/opt/parcx/lib/ -pthread
...
void GetSystemSettings() {
    ...
    try {
        conn = general.mysqlConnect("parcx_server");
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select  setting_name,setting_value from system_settings where setting_name in('facility_number','facility_name','cloud_download_webservice','cloud_operator_id')");
        while (res->next()) {
...
int main(void) {
    ...
    if (rc) {
        ...
        pid = fork();        
        if (pid > 0) {
            WriteToLog("Main: ", "Cloud Download Daemon Version : PX_CloudDownload_V01_03_02_20211006    Process Id:" + to_string(pid));
            cout << "PX_CloudDownload_V01_03_02_20211006" << "Process Id :" << pid << endl;
