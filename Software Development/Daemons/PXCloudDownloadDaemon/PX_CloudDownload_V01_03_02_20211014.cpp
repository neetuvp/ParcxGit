//g++ -std=c++11 -Wall -o PX_CloudDownload_V01_03_02_20211014 PX_CloudDownload_V01_03_02_20211014.cpp PX_GeneralOperations.cpp -ljsoncpp -lmysqlcppconn -lcurl -I/opt/lampp/include/ -L/opt/parcx/lib/ -pthread
...
//Fixed Segmentation fault when the damon starts
//Added review mode option
 */
...
int carpark_no, facility_no, device_no, cloud_operator_id,review_mode=1;
...
void WriteToLog(string function, string msg,int review) {
...
void WriteException(string function, string msg) {
    ...
    WriteToLog("****Exception****"+function,msg,0);
}
	...
	if(rs->rowsCount()>0)
	{
		while (rs->next()) {
		    if (rs->getString("setting_name") == "dayclosure_start_time") {
		        ...
		        WriteToLog("getDayclosuretime", "dayclosure_start_time :" + start_time,0);
		    } else {
		        end_time = rs->getString("setting_value");
		        WriteToLog("getDayclosuretime", "dayclosure_end_time :" + end_time,0);
		    }
        ...
        WriteToLog("getDayclosuretime", e.what(),0);
    }
...
int GetInterval() {
    ...
    WriteToLog("Time", "start:" + startdate + " end:" + enddate,1);  
    return seconds;
}
...
string generateTicketNumber(string facility_number, string carpark, string device, string ticketType) {
try {
    struct tm tm;
    ...
    return facility_number+"011100"+ticket;  
        ...
        WriteToLog("Generate Ticket number", e.what(),1);
        return "";
	} 
}

string CallWebservice(string jsonstring) { 
   // WriteToLog("","In CallWebservice",1);   
    string readBuffer = "";
    CURL *curl;
    CURLcode res;
    try {
        ...
        if (curl) {
            ...
            if (res == CURLE_OK) {
                ...
                WriteToLog("CallWebservice", "Curl Failed",0);
                WriteToLog("CallWebservice", "Request:" + jsonstring,0);
                WriteToLog("CallWebservice", "Return Code:" + to_string(res),0);
            }
            curl_easy_cleanup(curl);
        }
    }    catch (exception &e) {
        WriteToLog("CallWebservice", e.what(),0);
    }
    return readBuffer;
}
...
void GetSystemSettings() {
    ...
    try {
        conn = general.mysqlConnect("parcx_server");
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select  setting_name,setting_value from system_settings where setting_name in('facility_number','facility_name','cloud_download_webservice','cloud_operator_id','review_mode')");
	if(res->rowsCount()>0)
	{
		while (res->next()) {
		    ...
		    if (res->getString("setting_name") == "review_mode") {
		        review_mode = res->getInt("setting_value");
		    }
		}
	}
...
void DownloadCloudData(string response) {
    //WriteToLog("DownloadCloudData","In DownloadCloudData");
    sql::Connection *conn;
    sql::Statement *stmt;
    sql::ResultSet *rs;
    Json::Value array, accessidlist, updatejson, validationlist, reservationlist, platesewalletlist, parkingwalletlist,cooperatelist;
    string jsonstring;
    Json::FastWriter fw;
    bool parse;
    Json::Reader reader;
    ...
    int wallet_enabled,parking_spaces,date_validity,service_corporate_parker;
    ...
    try {
		...
		delete rs;
            ...
            for (i = 0; i < (signed)array["plates_ewallet"].size(); i++) {
                ...
                query = "Select id from plates_ewallet where user_id = " + to_string(user_id) + " and plate_number = '" + plate_number + "'";
                rs = stmt->executeQuery(query);
                if (rs->next()) 
                    query="update plates_ewallet set status="+to_string(status)+" where user_id = " + to_string(user_id) + " and plate_number = '" + plate_number + "'" ;
                else
            ...
            k = 0;
            for (i = 0; i < (signed)array["cooperate_users"].size(); i++) {
                result = 0;               
                user_id = array["cooperate_users"][i]["user_id"].asInt();
                user_name = array["cooperate_users"][i]["user_name"].asString();   
                date_validity=array["cooperate_users"][i]["date_validity"].asInt();
                start_date = array["cooperate_users"][i]["start_date"].asString();                
                expiry_date = array["cooperate_users"][i]["expiry_date"].asString();                
                parking_spaces = array["cooperate_users"][i]["parking_spaces"].asInt();                               
                status = array["cooperate_users"][i]["status"].asInt();
                service_corporate_parker=array["cooperate_users"][i]["service_corporate_parker"].asInt();
                ...
                query = "Select * from cooperate_users where user_id="+to_string(user_id);
                rs = stmt->executeQuery(query);
                if (rs->next())                                                           
                    {
                    if(service_corporate_parker==1)
                        query = "Update cooperate_users set parking_spaces = " +  to_string(parking_spaces) + ",date_validity="+to_string(date_validity)+",start_date="+start_date+",expiry_date="+expiry_date+",status="+to_string(status)+" where user_id = " + to_string(user_id);                                        
                    else
                        query = "Update cooperate_users set parking_spaces = 0,status=0 where user_id = " + to_string(user_id);
                    result = stmt->executeUpdate(query);
                    }   
                else                                                       
                    {
                    if(service_corporate_parker==1)
                        {
                        query = "INSERT INTO cooperate_users (user_id, user_name,date_validity,start_date,expiry_date,parking_spaces,  status) VALUES(" + to_string(user_id) + ", '" + user_name + "',"+to_string(date_validity)+","+start_date+","+expiry_date+", " + to_string(parking_spaces) + ", "+to_string(status)+")";
                        result = stmt->executeUpdate(query);
                        }
                    else
                        result=1;
            ...
            for (i = 0; i < (signed)array["parking_ewallet"].size(); i++) {
                ...
                if (rs->next()) {
                    ...
                    query = "INSERT INTO parking_ewallet (user_id, user_name, last_top_up_amount, last_deducted_amount, wallet_balance,last_payment_date_time,last_top_up_datetime,expiry_date,wallet_enabled) VALUES(" + to_string(user_id) + ", '" + user_name + "'," + last_top_up_amount + ", " + last_deducted_amount + ", " + wallet_balance + ",'" + last_payment_date_time + "','" + last_top_up_datetime + "','" + expiry_date + "'," + to_string(wallet_enabled) + ")";
                }
                delete rs;
                result = stmt->executeUpdate(query);
            ...
            if (cooperatelist.size() > 0) {
                UpdatePXCloudDownloadTime("cooperate_users");
            }
            
            


            if (cooperatelist.size()>0||accessidlist.size() > 0 || validationlist.size() > 0 || reservationlist.size() > 0 || platesewalletlist.size() > 0 || parkingwalletlist.size() > 0) {
                updatejson["task"] = 2;
                updatejson["facility_number"] = facility_no;
                updatejson["cloud_operator_id"] = cloud_operator_id;                
                updatejson["transaction_id"] = generateTicketNumber(to_string(facility_no), "0", "0", "00");
                ...
                updatejson["cooperateuserid"] = cooperatelist;               
                WriteToLog("DownloadCloudData", "Update ID Request:" + fw.write(updatejson),0);
                CallWebservice(fw.write(updatejson));
            }
            accessidlist = "";
        } else {
            WriteToLog("DownloadCloudData", "Parse Error",0);
        }
...
void CollectData(int interval) {
   // WriteToLog("","In CollectData"); 
    ...
    try {
        conn = general.mysqlConnect("parcx_server");
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select table_name,download_row_limit from pxcloud_download_settings where enabled=1 and day_closure=0 and time_interval = " + to_string(interval));
        Json::Value tb, tbrow;
	if(res->rowsCount()>0)
	{
		while (res->next()) {
		...
		request["table"] = tbrow;
		while (true) {
		    request["transaction_id"] = generateTicketNumber(to_string(facility_no), "0", "0", "00");
		    jsonstring = fw.write(request);
		    WriteToLog("CollectData", "Webservice Request:" + jsonstring,1);            
		    webservice_response = CallWebservice(jsonstring);
		    WriteToLog("CollectData", "Webservice Response:" + webservice_response,1);
...
void DCDaemon() {
    WriteToLog("DCDaemon", "DCDaemon Starts",0);
    ...
    try {
	    ...
	    WriteToLog("","Dayclosure request:"+jsonstring,0);
            if (request["table"].size() > 0) {                
                webservice_response = CallWebservice(jsonstring);
		WriteToLog("","Dayclosure response:"+webservice_response,0);
                DownloadCloudData(webservice_response);
            } else if (request["table"].size() == 0) {
                WriteToLog("DCDaemon", "No Records Found",0);
            }
        }
...
void DownloadDCData() 
    ...
    try
    {
        int SLEEP_INTERVAL;
        GetDayclosuretime();
        WriteToLog("Main", "Url:" + url,0);
        while (true) 
        {
            DCDaemon();
            SLEEP_INTERVAL = GetInterval(); //Add 30 min to dayclosure time;
            WriteToLog("DownloadDCData", "Daemon Sleeps for " + std::to_string(SLEEP_INTERVAL) + " seconds",0);
            WriteToLog("DownloadDCData", "StartDate:" + startdate + " EndDate:" + enddate,1);
	    //WriteToLog("","In DownloadDCData"); 
            sleep(SLEEP_INTERVAL);
            WriteToLog("DCDaemon", "Daemon Sleeps for 30 minutes more",0);
            sleep(30 * 60);       
        }
    }
    catch (exception &e) {        
        WriteException("DownloadDCData", e.what());
    }
}
...
void GetCloudDownloadSettings() {
    ...
    try {
        conn = general.mysqlConnect("parcx_server");
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select distinct(time_interval) from pxcloud_download_settings where day_closure=0 and enabled=1");
        n = res->rowsCount();
        ...
        delete res; 
        WriteToLog("CloudDownload", "Created Realtime thread",0);
        ...
        if (res->next())             
            ...
            WriteToLog("CloudDownload", "Created DC thread",0);
            }
        delete res;        
        ...
        WriteToLog("CloudDownload", "JOINING THREADS",0);
...
int main(void) {
    ...
    if (rc) {
        if (EWOULDBLOCK == errno) {
            WriteToLog("Main: ", "Another Instance is running",0);            
        }
    } else {
        pid_t pid, sid;
        pid = fork();        
        if (pid > 0) {
            WriteToLog("Main: ", "Cloud Download Daemon Version : PX_CloudDownload_V01_03_02_20211014    Process Id:" + to_string(pid),0);
            cout << "PX_CloudDownload_V01_03_02_20211014" << "Process Id :" << pid << endl;
        ...
        WriteToLog("Main", "Successfully started Cloud download daemon",0);
        sid = setsid();
        if (sid < 0) {
            exit(EXIT_FAILURE);
        }
