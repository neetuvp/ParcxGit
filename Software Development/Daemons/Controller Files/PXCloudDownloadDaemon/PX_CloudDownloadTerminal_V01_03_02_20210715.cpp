//g++ -std=c++11 -Wall -o PX_CloudDownloadTerminal_V01_03_02_20210715 PX_CloudDownloadTerminal_V01_03_02_20210715.cpp PX_GeneralOperations.cpp -ljsoncpp -lmysqlcppconn -lcurl -I/opt/lampp/include/ -L/opt/parcx/lib/ -pthread
...
void GetDayclosuretime() {
    ...
    try {
        ...
        rs = stmt->executeQuery("Select setting_name,setting_value from device_settings where setting_name in ('dayclosure_start_time','dayclosure_end_time')");
        while (rs->next()) {
...
void GetSystemSettings() {
    ...
    try {
        conn = general.mysqlConnect(DATABASE);
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select  setting_name,setting_value from device_settings where setting_name in('facility_number','facility_name','cloud_download_webservice','cloud_operator_id')");
        while (res->next()) {
...
void DownloadCloudData(string response) {
    ...
    int mobile_app_user_id, payment_flag, cancel_flag, used_flag;
        ...
        if (parse) {
            ...
            for (i = 0; i < (signed)array["reservation"].size(); i++) {
                ...
                if (rs->next()) {
                    ...
                    if (rs->getInt("cloud_upload_status") == 1) {
                        query = "Update parking_reservation set plate_number = '" + plate_number + "',ticket_id = '" + ticket_id + "',name = '" + name + "',address = '" + address + "',country  = '" + country + "',phone='" + phone + "',email = '" + email + "',parking_bay_number='" + parking_bay_number + "',from_time='" + from_time + "',to_time = '" + to_time + "',reserved_parking_duration='" + reserved_parking_duration + "',reserved_parking_fee='" + reserved_parking_fee + "',payment_reference_id='" + payment_reference_id + "',payment_flag=" + to_string(payment_flag) + ",cancel_flag=" + to_string(cancel_flag) + " where reservation_id = " + to_string(rowid);
                        result = stmt->executeUpdate(query);
                    }
                } else {
                    query = "INSERT INTO parking_reservation (mobile_app_user_id, operator_id, transaction_id, plate_number, ticket_id, name, address, country, phone, email, parking_bay_number, from_time, to_time, creation_date_time, reserved_parking_duration, reserved_parking_fee, payment_reference_id, payment_flag, cancel_flag, used_flag,cloud_upload_status) VALUES(" + to_string(mobile_app_user_id) + ", " + to_string(operator_id) + ", '" + transaction_id + "', '" + plate_number + "', '" + ticket_id + "', '" + name + "', '" + address + "', '" + country + "', '" + phone + "', '" + email + "','" + parking_bay_number + "', '" + from_time + "', '" + to_time + "', '" + creation_date_time + "', '" + reserved_parking_duration + "', '" + reserved_parking_fee + "', '" + payment_reference_id + "', " + to_string(payment_flag) + ", " + to_string(cancel_flag) + ", " + to_string(used_flag) + ",1)";
                    result = stmt->executeUpdate(query);
                }
            ...
            if (cooperatelist.size()||accessidlist.size() > 0 || validationlist.size() > 0 || reservationlist.size() > 0 || platesewalletlist.size() > 0 || parkingwalletlist.size() > 0) {
                updatejson["task"] = 2;
	...
	WriteException("DownloadCloudData", query);
        delete stmt;
        delete conn;
...
int main(void) {
    ...
    if (rc) {
        ...
        pid = fork();        
        if (pid > 0) {
            WriteToLog("Main: ", "Cloud Download Daemon Version : PX_CloudDownloadTerminal_V01_03_02_20210715    Process Id:" + to_string(pid));
            cout << "PX_CloudDownloadTerminal_V01_03_02_20210715" << "Process Id :" << pid << endl;
