...
#define MediaPath "/opt/lampp/htdocs/GreetingStage/Media"
using namespace std;
...
Php::Value updateStage(Php::Value data)
{
    ...
    try
        ...
        query_string = "Update greeting_screen set message_line1=?,message_line2=?,message_line3=?,description=?,bg_video_file=?,animation_file=?,title=?,animation_type=?,timeout_period=?, last_updated_date_time=NOW() where stage_id=?";
