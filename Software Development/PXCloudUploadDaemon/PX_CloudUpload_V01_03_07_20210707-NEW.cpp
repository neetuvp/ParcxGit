//sudo g++ -std=c++11 -Wall -o PX_CloudUpload_V01_03_07_20210707 PX_CloudUpload_V01_03_07_20210707-NEW.cpp PX_GeneralOperations.cpp -ljsoncpp -lmysqlcppconn -lpthread -lcurl -I/opt/lampp/include/ -L/opt/parcx/lib/
//NEW DAEMON
...
void GetSystemSettings() {
    ...
    try {
        ...
        res = stmt->executeQuery("Select  setting_name,setting_value from system_settings where setting_name in('dayclosure_start_time','dayclosure_end_time','cloud_operator_id','facility_number','facility_name','cloud_upload_webservice')");
        while (res->next()) {          
...
Json::Value TableList(Json::Value tables,int n) {
    ...
    try 
        ...
        for(i=0;i<n;i++)
            ...
            db=tables[i]["db_name"].asString();
            table_id=tables[i]["table_id"].asString();
            rowlimit=tables[i]["rowlimit"].asInt();
            task=tables[i]["task"].asInt();
            
            res = stmt->executeQuery("Select * from "+db+"."+table+" where cloud_upload_status=0 limit " + to_string(rowlimit));
            if (res->rowsCount() > 0) 
                {
                array.clear();
                row.clear();
                ...
                while (res->next()) 
                    ...
                    if(row.isMember("flag"))
                        row["reproccessing_flag"] = row["flag"];    
                    
                    if(table=="access_whitelist")
                        {
...
void UpdateUploadStatus(Json::Value json) 
    ...
    sql::Connection *con;
    sql::Statement *stmt;
    string id_string,table,db,table_id;
    try 
        {                
        ...
        for (int i = 0; i < (signed)json.size(); i++) 
            ...
            db=json[i]["db_name"].asString();
            table_id=json[i]["table_id"].asString();
            id_string=json[i]["id_string"].asString();            
            if(id_string!="")
                stmt->executeUpdate("Update "+db+"."+table+" set cloud_upload_status = 1 where "+table_id+" in (" + id_string + ")");            
            if(table=="open_transactions")
                {
                id_string=json[i]["delete_id_string"].asString(); 
                if(id_string!="")
                    stmt->executeUpdate("Delete from "+db+".open_transactions where id in (" + id_string + ") and movement_type=2");
                
                }
            
            stmt->executeUpdate("Update parcx_server.pxcloud_upload_settings set cloud_upload_date_time=CURRENT_TIMESTAMP() where table_name = '" + table + "'");
            }
        delete stmt;
        delete con;
        }    
...
void ProcessRequest(string jsonstring,Json::Value database) 
    {
    bool parse;
    int j = 0, i = 0;
    Json::Reader reader;   
        ...
        if (webservice_response > "") 
            ...
            if (parse) 
                ...
                for (j = 0; j < (signed)array["response"].size(); j++) 
                    ...
                    json[j]["db_name"]=database[table_name]["db_name"].asString();
                    json[j]["table_id"]=database[table_name]["table_id"].asString();
                    json[j]["id_string"]=id_string;  
                    
                    if(table_name=="open_transactions")
                        {
                        id_string = "";
                        ...
                        json[j]["delete_id_string"]=id_string;  
                        }
                    }
                if (!json.isNull())
...
void PostRealTimeData(int interval) 
    ...
    Json::Value json,tables,database; 
    Json::FastWriter fw;   
    int n=0,i=0;
    string jsonstring;
    ...
    try 
        ...
        conn = general.mysqlConnect("parcx_server");
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select table_name,db_name,table_id,task,upload_row_limit from pxcloud_upload_settings where day_closure = 0 and enabled=1 and time_interval = " + to_string(interval));
        n = res->rowsCount();                
        while (res->next()) 
            {
            tables[i]["table_name"] = res->getString("table_name").asStdString();
            tables[i]["db_name"] = res->getString("db_name").asStdString();
            tables[i]["table_id"] = res->getString("table_id").asStdString();
            tables[i]["task"] = res->getInt("task");
            tables[i]["rowlimit"] = res->getInt("upload_row_limit");
            database[res->getString("table_name").asStdString()]["db_name"]=res->getString("db_name").asStdString();
            database[res->getString("table_name").asStdString()]["table_id"]=res->getString("table_id").asStdString();
            i++;
            }
        ...
        while (true) 
            ...
            try
                ...
                if (json["table"].size() > 0) 
                    ...
                    ProcessRequest(jsonstring,database);
                    } 
...
void PostDCData() 
    ...
    try 
        ...
        conn = general.mysqlConnect("parcx_server");
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select table_name,db_name,task,upload_row_limit,table_id from pxcloud_upload_settings where day_closure = 1 and enabled=1");
        n = res->rowsCount();                
        while (res->next()) 
            {
            tables[i]["table_name"] = res->getString("table_name").asStdString();
...
int main(void) 
    ...
    if (rc) 
        ...
        if (pid > 0) 
            {
            WriteToLog("Main: ", "Cloud Upload Daemon Version : PX_CloudUpload_V01_03_07_20210707    Process Id:" + proccess_id);
            cout << "Cloud Upload Daemon Version : PX_CloudUpload_V01_03_07_20210707" << "Process Id :" << proccess_id << endl;                       
