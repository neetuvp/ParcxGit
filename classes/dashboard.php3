...
class dashboard
    ...
    function revenue_lastdays($facility_number,$carpark_number)
        {        
        $con=$this->db_connect_reporting();
        $query_string="select report_date,report_day,sum(gross_amount) as gross_amount from summary_daily_revenue where carpark_number=".$carpark_number." and report_date between DATE(NOW())-INTERVAL 7 DAY and DATE(NOW())- INTERVAL 1 DAY group by report_date ";
        ...
        mysqli_close($con);
        $con=$this->db_connect();
        $query_string="select sum(gross_amount) as gross_amount from parking_revenue where facility_number=".$facility_number." and carpark_number=".$carpark_number;
        $result=mysqli_query($con,$query_string) or die(mysqli_error($con));
        if($data=mysqli_fetch_assoc($result))
            {
            $temp=[
        ...
        if(mysqli_num_rows($result)== 1)
            ...
            while ($data = mysqli_fetch_assoc($result)) 
               ...
               // $html_data .= '</div>';//end row
                ...
                $html_data .= '</div>';
                $html_data .= '<a href="occupancy_detail.php?carpark_number='.$data['carpark_number'].'"><button type="button" class="btn btn-block bg-secondary-gradient" id="ShowCarparkDetail" facility_number="'.$facility_number.'" carpark_number="'.$data['carpark_number'].'">More <i class="fa fa-arrow-circle-right"></i></button></a>'; 
                $html_data .= '</div>';   
