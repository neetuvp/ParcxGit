            ...
            if (status_message == "Access Allowed") {
                if (stoi(movement_type) == 1) {
                    ...
                    if (short_term_entry_after_contract_parking_space_exceeded == 1 && full_payment_after_contract_parking_space_exceeded == 0) {
                        sql::ResultSet *res;
                        if (plate_number != "")
                            query = "select * from parcx_server.access_whitelist where plate_number='" + plate_number + "'";
                        else
                            query = "select * from parcx_server.access_whitelist where ticket_id='" + ticket_id + "'";
                        res = stmt->executeQuery(query);
                        res->next();
                        cooperate_parker = res->getInt("corporate_parker");
                        if (cooperate_parker > 1) {
                            parking_spaces = 0;
                            occupied = 0;
                            query = "select * from parcx_server.cooperate_users where  user_id=" + to_string(cooperate_parker);
                            res = stmt->executeQuery(query);
                            if (res->next())
                                parking_spaces = res->getInt("parking_spaces");
                            ...
                            res = stmt->executeQuery(query);
                            if (res->next())
                                occupied = res->getInt("count");

                            WriteToLog("UploadParkingAccessMovements : ticketId: " + ticket_id + "\tPlate number: " + plate_number + "\tcooperate_parker: " + to_string(cooperate_parker) + "\tparking_spaces: " + to_string(parking_spaces) + "\tOccupiend: " + to_string(occupied));

                            if (occupied < parking_spaces) {
                                query = "SELECT b.id as id,b.ticket_id as ticket_id,b.plate_number as plate_number  FROM parcx_server.access_whitelist a,parcx_reporting.open_transactions b where a.ticket_id=b.ticket_id  and a.plate_number=b.plate_number and b.entry_type=1 and a.corporate_parker=" + to_string(cooperate_parker) + " order by b.entry_date_time";
                                res = stmt->executeQuery(query);
                                if (res->next()) {
                                    id = res->getString("id");
                                    ticket_id = res->getString("ticket_id");
                                    plate_number = res->getString("plate_number");
                                    WriteToLog("UploadParkingAccessMovements : changing entry type of ticket id: " + ticket_id + " plate_number: " + plate_number + " to 2");
                                    query = "update open_transactions set entry_type=2,pay_upto=current_timestamp where id=" + id;
                                    result = stmt->executeUpdate(query);
                                }
                            }
                        }
...
Php::Value UploadPayments(Php::Value request) {
    ...
    try {
        ...
        for (int i = 0; i < request.size(); i++) {
            ...
            string coupon_value = request[i]["coupon_value"];
