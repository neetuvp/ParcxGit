...
Json::Value PX_Plate_Suggestions_GetPlateSuggestions(string plateNo){
	 ...
	 if (parseSuccess)
  {
	...
	switch(device_type){
		...
		if(json["get_plate_suggestions"].asInt()==1){
			...
			res["plate_suggestions"]=PX_Plate_Suggestions_GetPlateSuggestions(json["plate_number"].asString());
		}
		else{
			res["error"]=false;
			p.writeLog(log_file_path,"PaymentTicketCheck","Request from cashier to get the details for plate_number: "+json["plate_number"].asString());
			res["plate_suggestions"]=PX_Plate_Suggestions_GetPlateSuggestions(json["plate_number"].asString());
		}
