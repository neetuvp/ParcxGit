...
int deductMoneyFromWallet,carparkNumber,facilityNumber,deviceNumber,deviceType,task,entryGrace,exitGrace,reservationEnabled,accessEnabled,deviceFunction,seconds,anprEnabled,cameraId,plateCapturedInterval,plateCapturedId,validationEnabled,walletEnabled,entryType;
	...
	try
		...
		if(res->next())
			...
			if(carparkNumber!=res->getInt("carpark_number"))
				...
				{								
				loadSystemSettings();	
				if(deviceType!=2 || (walletEnabled==1 && deviceType==2))				
					{
