...
string log_file_path = "/opt/parcx/Logs/Services/PXPlateSuggestions/ApplicationLog-PX-PlateSuggestions-" + p.currentDate() + ".log";
string error_file_path = "/opt/parcx/Logs/Services/PXPlateSuggestions/Exception-PX-PlateSuggestions-" + p.currentDate() + ".log";

Json::Value PX_Plate_Suggestions_GetPlateSuggestions(string plateNo, int plate_suggestions)
{
	...
	try
	{
		...
		if (plate_suggestions == 1)
		{
			query = "SELECT device_number,plate_number,ticket_id,entry_type,entry_date_time from open_transactions where plate_number like '%" + plate_number + "%'";
		}
		else{
			query = "SELECT device_number,plate_number,ticket_id,entry_type,entry_date_time from open_transactions where plate_number like '%" + plate_number + "%' order by id desc limit 1";
		}
	...
	try
	{
		...
		if (parseSuccess)
		{
			...
			case 5:
				...
				//if(json["get_plate_suggestions"].asInt()==1){
				res["error"] = false;
...
extern "C"
{
	...
	{
		static Php::Extension extension("PX_PlateSuggestions", "1.0");
