//11-03-2022 Remove Physical Cash and Over/Short rows from Shift Report when Physical Cash is 0
//08-04-2022 Added an additional query to get the sum of gross_amount from revenue_payments table for Revenue Payments Report as the rounding value was not accurate in PUMC
...
void RevenueReport::paymentTransactions(Php::Value json) {
    try {
        ...
        string condition="",total_query="",total="0";
        ...
        if (showvoid == 0)
            condition = condition + " and (void is null or void='')";
        else
            condition = condition + " and void is not null";

        if (carpark.length() > 0)
            condition = condition + " AND carpark_number in(" + carpark + ")";
        if (device.length() > 0)
            condition = condition + " AND device_number in(" + device + ")";

        if (type.length() > 0)
            condition = condition + " AND payment_type IN(" + type + ")";

        if (discount.length() > 0)
            condition = condition + " AND discount_id IN(" + discount + ")";  
        
        if (validation_id.length() > 0)
            condition = condition + " AND validation_id!=''";
        ...
        if (category.length() > 0) {
            for (auto &iter : category) {
                ...
                if (iter.second == 1)
                    condition = condition + " (parking_fee>0 and lost_fee=0 and discount_amount=0)";
                if (iter.second == 2)
                    condition = condition + " lost_fee>0";
                if (iter.second == 3)
                    condition = condition + " discount_amount>0";
                if (iter.second == 4)
                    condition = condition + " (discount_amount=0 AND gross_amount=0 and validation_value='' and coupon_value='')";
                if (iter.second == 5)
                    condition = condition + " payment_category='ProductSale'";
            }
            condition = condition + " )";
        }
	int decimal = getDecimal();
	total_query = "Select ROUND(sum(gross_amount),"+to_string(decimal)+") as gross_amount from revenue_payments where payment_date_time between '" + startdate + "' AND '" + enddate + "'";
	total_query = total_query + condition;
	res = rStmt->executeQuery(total_query);
	if(res->next())
	{
		total = res->getString("gross_amount");
	}
	delete res;

        sql_query = sql_query + condition + " ORDER BY  payment_date_time desc";
		...
		Php::out << "<h3>" << currency << " " << total << "</h3>" << endl;
        ...
        if (res->next()) {
            ...
            if (res->getString("validation_id") != "") {
                ...
                if(stoi(outputArr[0])>0)
                    ...
                    if(stoi(outputArr[1])>0)
                        validation=validation+outputArr[1]+" % ";
                html_data += "<hr><div class='row'><div class='col'>" + toString(label["validations"]) + "</div><div class='col'>"+validation+ "</div></div>";
            }
    ...
    try {
        ...
        if (res->next()) {
            ...
            if (res->getString("validation_id") != "") {
                ...
                html_data += "<td><span>" + validation+"</span></td></tr>";
            }
            if (res->getString("coupon_id") != "") {
                html_data += "<tr><td></td>";
...
void RevenueReport::shiftReport(Php::Value json) {
    ...
    try {
        ...
        if (res->rowsCount() > 0) {
            ...
            while (res->next()) {
                i++;
                /*difference = 0;    //Commented for PUMC as physical cash is disabled
