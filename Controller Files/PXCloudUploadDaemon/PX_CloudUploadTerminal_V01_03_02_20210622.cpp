...
int device_number,carpark_number;
...
void GetSystemSettings() {
    ...
    try {
        conn = general.mysqlConnect();
        stmt = conn->createStatement();
        res = stmt->executeQuery("Select  setting_name,setting_value from device_settings where setting_name in('cloud_operator_id','facility_number','facility_name','cloud_upload_webservice','device_name','device_number','carpark_name','carpark_number')");
        while (res->next()) {
	    ...
	    if (res->getString("setting_name") == "device_name") {
                device_name = res->getInt("setting_value");
            }
	    ...
	    if (res->getString("setting_name") == "carpark_name") {
                carpark_name = res->getInt("setting_value");
            }
        }
                ...
                for (j = 0; j < (signed)array["response"].size(); j++) 
                    ...
                    if (array["response"][j]["table"] == "parking_movements_access") 
                        ...
                        if (id_string > "") 
                            ...
                            UpdateUploadStatus(sql,"parking_movements_access");                            
                            }
                        } 
                        ...
                        if (id_string > "") 
                            ...
                            UpdateUploadStatus(sql,"parking_movements_manual");                            
                            }
                        } 
                        ...
                        if (id_string > "") 
                            ...
                            UpdateUploadStatus(sql,"parking_reservation");                            
                            }
                        } 
                        ...
                        if (id_string > "") 
                            ...
                            UpdateUploadStatus(sql,"contract_parking_transactions");                            
                            }
                        }
                    }
                } 
...
void CollectData(int interval) 
    ...
    json["carpark_number"] = carpark_number;
    try 
        {
        conn = general.mysqlConnect();
        stmt = conn->createStatement();
...
int main(void) 
    ...
    if (rc) 
        ...
        if (pid > 0) 
            {
            WriteToLog("Main: ", "Cloud Upload Daemon Version : PX_CloudUploadTerminal_V01_03_02_20210622    Process Id:" + to_string(pid));
            cout << "Cloud Upload Daemon Version : PX_CloudUploadTerminal_V01_03_02_20210622" << "Process Id :" << pid << endl;
